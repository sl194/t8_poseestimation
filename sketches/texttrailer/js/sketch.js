  ...
  if (poses) {
    poses.forEach((singlePose) => {
      ...
      if (sketchGuiState.visual == "trail") {
        ...
        // For all past poses... --> this is the thing that makes it have the trail even when not moving
        let wordIndex = 0;
        ...
        for (let p = 0; p < pastPaintedPoses.length; p+3) {
  ...
  poseArray.forEach((singlePose) => {
    ...
    if (sketchGuiState.visual == "trail") {
      ...
      if (sketchGuiState.showVideo) {
        ...
        //this part allows the text to be stacked when it is p++
